...
<body>
  ...
  ?>
  <div class="login">
  	<h1>Arme su consulta</h1>
    <form action="conectar.php" method="post">
      <?php for ($i=0; $i < $cantidad['cantidad']; $i++):?>
        <select class="" name="articulos[]">
        <?php $query = $mysqli -> query ("SELECT id,CONCAT(codigo,' - ',descripcion) AS articulo FROM producto");
            while ($valores = mysqli_fetch_array($query)):?>
              <option value="<?=$valores['id']?>"><?=$valores['articulo']?></option>';
            <?php endwhile;?>
        </select>
        <input type="number" name="cantidad[]" min="0" value="0" required><br>
      <?php endfor; ?>
      <br>
